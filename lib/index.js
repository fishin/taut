...
exports.startQueue = function () {
    ...
    var queueObj = setInterval(function () {
        ...
        if (internals.queue.length > 0) {
            ...
            var totalActive = internals.getActiveTotal();
            ...
            if (!totalActive.activeJobs[jobId] && pr === null && totalActive.total <= size) {
                internals.Queue.settings.startJob(jobId, pr, Hoek.ignore);
                internals.Queue.removeJob(jobId, pr);
            } else if (pr !== null && !totalActive.activePRs[jobId] && totalActive.total <= size) {
                internals.Queue.settings.startJob(jobId, pr, Hoek.ignore);
                internals.Queue.removeJob(jobId, pr);
            } else if (pr !== null && totalActive.activePRs[jobId] && !totalActive.activePRs[jobId].prs[pr] && totalActive.total <= size) {
                internals.Queue.settings.startJob(jobId, pr, Hoek.ignore);
                internals.Queue.removeJob(jobId, pr);
            }
//            else {
