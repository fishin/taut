...
exports.startQueue = function () {
    ...
    var queueObj = setInterval(function () {
        ...
        if (internals.queue.length > 0) {
            ...
            if (!activeJobs[jobId] && pr === null && totalActive < size) {
                internals.Queue.settings.startJob(jobId, pr, Hoek.ignore);
                internals.Queue.removeJob(jobId, pr);
                totalActive++;
            }
//            else {
//                console.log('job all full');
//            }
            //  start up prs
            if (pr !== null && !activePRs[jobId] && totalActive < size) {
                internals.Queue.settings.startJob(jobId, pr, Hoek.ignore);
                internals.Queue.removeJob(jobId, pr);
                totalActive++;
            } else if (pr !== null && activePRs[jobId] && !activePRs[jobId].prs[pr] && totalActive < size) {
                internals.Queue.settings.startJob(jobId, pr, Hoek.ignore);
                internals.Queue.removeJob(jobId, pr);
                totalActive++;
            }
//            else {
//                console.log('pr all full');
//            }
        }
//        else {
